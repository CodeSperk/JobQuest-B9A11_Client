...
const MyJobs = () => {
  ...
  if(isPending){
   ...
   const handleRemoveCoffee = (deletedJobId) => {
...
<div className="text-center">
  <h1 className="mb-4">
  Manage Your Job Listings: <br /> <span className="text-[var(--clr-focused)]">Edit and Remove Jobs </span>
    <br /> as Needed
  </h1>
</div>
</div>
...
{
  myJobs.length > 0 ? ( <div className="p-2 mx-auto sm:p-4">
  <div className="overflow-x-auto">
    <table className="min-w-full mt-8">
      ...
      <tbody>
           {myJobs.map(job => <tr key={job._id} className="border-b border-opacity-20">
            <td className="p-3">
            <img src={job?.pictureURL} alt="" className="h-12 w-auto rounded"/>
          </td>
          ...
          <td className="p-3">
            ...
            <button className="px-3 py-1 font-semibold rounded-md bg-[var(--clr-focused)] text-[var(--clr-light)] text-xl hover:scale-90 duration-300" onClick={() => handleDeleteJob(job._id)}>
            <MdDeleteOutline />
            </button>                  
  ...
  </div> )          : <div className="min-h-96 flex flex-col justify-center items-center text-center gap-4"> <h1> <span className="text-[var(--clr-focused)]"> No Jobs Found. </span></h1> <p className="text-xl">You Have added no jobs yet.</p> </div> 
}
