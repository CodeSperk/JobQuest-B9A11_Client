...
const JobCategory = ({jobs}) => {
  ...
  return (
    <section>
      <SecHeader name="Category" title="Discover Jobs in Your Field"></SecHeader>
    ...
    <TabList>
      {
        categories.map((category, idx) => <Tab key={idx} onClick={() => handleJobCategory(category)}>{category}</Tab>)
      }
    </TabList>
    {
      categories.map((category, idx) => 
        <TabPanel key={idx}>
          ...
          {
            targetedJobs.map(job => <div key={job._id} className="card-style flex flex-col md:flex-row gap-6 p-6 md:p-8 border rounded-md hover:border-[var(--clr-focused)]">
              ...
              <div className="flex-1 space-y-4">
                ...
                <div className="flex gap-6 justify-between">
                  <p className="text-[var(--clr-focused)] text-lg font-bold">{job?.salaryRange}</p>
                  <p><span className="font-bold">Job Posted</span> : {job?.jobPostingDate}</p>
                  <p><span className="font-bold"> Deadline</span> : {job?.applicationDeadline}</p>
                </div>
              </div>
              <div>
                <div>
      ...
      <Button2 text="Post a job"></Button2>
        ...
        <div className="p-8 border rounded-xl shadow-xl space-y-6 card-style md:w-1/2 lg:w-full">
          ...
          <div className="space-y-4">
            ...
            <BsMicrosoft className="p-2 text-5xl bg-[var(--clr-light)] border-2 rounded-md"/><p className="flex-1">Microsoft</p> <FaLongArrowAltRight className="text-2xl"/>
            </div>
            {/* google */}
            <div className="flex items-center gap-2 hover:text-[var(--clr-focused)] cursor-pointer pr-4 hover:pr-0">
            <BsGoogle className="p-2 text-5xl bg-[var(--clr-light)] border-2 rounded-md"/><p className="flex-1">Google</p> <FaLongArrowAltRight className="text-2xl"/>
            </div>
            {/* Facebook */}
            <div className="flex items-center gap-2 hover:text-[var(--clr-focused)] cursor-pointer pr-4 hover:pr-0">
            <BsFacebook className="p-2 text-5xl bg-[var(--clr-light)] border-2 rounded-md"/><p className="flex-1">Facebook</p> <FaLongArrowAltRight className="text-2xl"/>
            </div>
          </div>
        </div>
      </aside>
